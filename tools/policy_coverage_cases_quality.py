...
def quality_cases(module: ModuleType) -> list[tuple[str, str, str | None, Callable[[Path], None]]]:
    ...
    def _(root: Path) -> None:
        mutate_labels(module, root, lambda d: d["core"][1].__setitem__("name", d["core"][0]["name"].upper()))
    ...
    def _(root: Path) -> None:
        mutate_labels(module, root, lambda d: d["overlays"]["domain"].__setitem__("Bad_Name", []))
    ...
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8").replace('name: "Bug report"', 'name: ""')
        module._write_fixture(root / form_path, text)

    @case("issue-form-title", "issue-forms", "Top-level title")
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8").replace('title: "[Bug]: "', 'title: "[Wrong]: "')
        module._write_fixture(root / form_path, text)

    @case("issue-form-labels", "issue-forms", "Top-level labels")
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8").replace('labels: ["bug"]', 'labels: ["wrong"]')
        module._write_fixture(root / form_path, text)

    @case("issue-form-body-count", "issue-forms", "between 1 and 10")
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8")
        module._write_fixture(root / form_path, re.sub(r"(?ms)^body:\n.*\Z", "body:\n", text))

    @case("issue-form-unsupported-type", "issue-forms", "Unsupported issue-form")
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8").replace("  - type: markdown", "  - type: invalid", 1)
        module._write_fixture(root / form_path, text)

    @case("issue-form-duplicate-id", "issue-forms", "IDs must be unique")
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8").replace("    id: summary", "    id: profile")
        module._write_fixture(root / form_path, text)

    @case("issue-form-missing-id", "issue-forms", "element IDs are missing")
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8").replace("    id: summary\n", "")
        module._write_fixture(root / form_path, text)

    @case("issue-form-security-guidance", "issue-forms", "route vulnerability")
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8").replace("SECURITY.md", "SECURITY-X.md")
        module._write_fixture(root / form_path, text)

    @case("issue-form-required-field", "issue-forms", "must be mandatory")
    def _(root: Path) -> None:
        text = (root / form_path).read_text(encoding="utf-8").replace("      required: true", "      required: false", 1)
        module._write_fixture(root / form_path, text)
    ...
    def _(root: Path) -> None:
        text = (root / form_config).read_text(encoding="utf-8").replace("blank_issues_enabled: false", "blank_issues_enabled: true")
        module._write_fixture(root / form_config, text)

    @case("issue-config-url", "issue-forms", "Private-security contact URL")
    def _(root: Path) -> None:
        text = (root / form_config).read_text(encoding="utf-8").replace("https://github.com/example/fixture/security/policy", "https://example.invalid/security")
        module._write_fixture(root / form_config, text)

    @case("issue-config-private", "issue-forms", "require private reporting")
    def _(root: Path) -> None:
        text = (root / form_config).read_text(encoding="utf-8").replace("private", "restricted")
        module._write_fixture(root / form_config, text)
    ...
    def _(root: Path) -> None:
        (root / "src/modules/Quality.bas").unlink()

    @case("vba-export-no-name", "vba-export-header", "must declare Attribute VB_Name")
    def _(root: Path) -> None:
        rewrite_vba(module, root, "src/modules/Quality.bas", "Option Explicit\nPublic Function Echo(ByVal value As String) As String\nEcho = value\nEnd Function\n")

    @case("vba-export-duplicate-name", "vba-export-header", "more than once")
    def _(root: Path) -> None:
        text = (root / "src/modules/Quality.bas").read_bytes().decode("cp1252").replace("\r\n", "\n")
        rewrite_vba(module, root, "src/modules/Quality.bas", text.replace("Option Explicit", 'Attribute VB_Name = "Quality"\nOption Explicit'))

    @case("vba-export-name-not-line1", "vba-export-header", "must be line 1")
    def _(root: Path) -> None:
        text = (root / "src/modules/Quality.bas").read_bytes().decode("cp1252").replace("\r\n", "\n")
        rewrite_vba(module, root, "src/modules/Quality.bas", "' comment\n" + text)

    @case("vba-export-name-not-leading", "vba-export-header", "leading export header")
    def _(root: Path) -> None:
        text = (root / "src/modules/Quality.bas").read_bytes().decode("cp1252").replace("\r\n", "\n")
        lines = text.splitlines()
        rewrite_vba(module, root, "src/modules/Quality.bas", "\n".join(["' pad"] * 25 + [lines[0]] + lines[1:]) + "\n")

    @case("vba-export-invalid-identifier", "vba-export-header", "not a VBA identifier")
    def _(root: Path) -> None:
        text = (root / "src/modules/Quality.bas").read_bytes().decode("cp1252").replace("\r\n", "\n")
        rewrite_vba(module, root, "src/modules/Quality.bas", text.replace('"Quality"', '"Bad-Name"', 1))

    @case("vba-export-name-too-long", "vba-export-header", "character limit")
    def _(root: Path) -> None:
        text = (root / "src/modules/Quality.bas").read_bytes().decode("cp1252").replace("\r\n", "\n")
        rewrite_vba(module, root, "src/modules/Quality.bas", text.replace('"Quality"', '"' + "Q" * 32 + '"', 1))

    @case("vba-export-name-collision", "vba-export-header", "collides")
    def _(root: Path) -> None:
        rewrite_vba(module, root, "src/modules/Other.bas", 'Attribute VB_Name = "quality"\nOption Explicit\nPublic Sub Other()\nEnd Sub\n')
    ...
    def _(root: Path) -> None:
        rewrite_vba(module, root, "src/modules/Extra.bas", 'Attribute VB_Name = "Extra"\nOption Explicit\nPublic Sub X()\nEnd Sub\n')
    ...
    def _(root: Path) -> None:
        def mutation(d: dict) -> None:
            d["vba"]["components"]["src/modules/Missing.bas"] = "public"
            d["vba"]["components"] = dict(sorted(d["vba"]["components"].items(), key=lambda item: item[0].casefold()))
    ...
    def _(root: Path) -> None:
        mutate_config(module, root, lambda d: d["vba"]["components"].__setitem__("tests/modules/QualityTests.bas", "public"))

    @case("vba-source-root-test-role", "vba-visibility", "must not be stored under a source root")
    def _(root: Path) -> None:
        mutate_config(module, root, lambda d: d["vba"]["components"].__setitem__("src/modules/Quality.bas", "test"))

    @case("generated-required-unregistered", "generated-vba-contract", "requires this")
    def _(root: Path) -> None:
        mutate_config(module, root, lambda d: d["vba"]["components"].pop("src/modules/Quality.bas"))

    @case("generated-required-wrong-role", "generated-vba-contract", "requires role")
    def _(root: Path) -> None:
        mutate_config(module, root, lambda d: d["vba"]["components"].__setitem__("src/modules/Quality.bas", "internal"))

    @case("generated-required-not-tracked", "generated-vba-contract", "starter component to be tracked")
    def _(root: Path) -> None:
        def mutation(d: dict) -> None:
            contract = d["profiles"]["library"]["vba_contract"]
            contract["required_components"].pop("src/modules/Quality.bas")
            contract["required_components"]["src/modules/Missing.bas"] = "public"
            contract["required_components"] = dict(sorted(contract["required_components"].items()))
            d["vba"]["components"]["src/modules/Missing.bas"] = "public"
            d["vba"]["components"] = dict(sorted(d["vba"]["components"].items()))
        mutate_config(module, root, mutation)

    @case("legacy-api-collision", "vba-public-api", "collides")
    def _(root: Path) -> None:
        rewrite_vba(module, root, "src/modules/Other.bas", 'Attribute VB_Name = "Other"\nOption Explicit\nPublic Function Echo() As String\nEnd Function\n')
        module._run_git(root, "add", "src/modules/Other.bas")
        def mutation(d: dict) -> None:
            d["vba"]["components"]["src/modules/Other.bas"] = "public"
            d["vba"]["components"] = dict(sorted(d["vba"]["components"].items(), key=lambda item: item[0].casefold()))
        mutate_config(module, root, mutation)
