...
def scorecard_git_input(published: str) -> bool:
    """Bind file_mode to the Scorecard action's block-style with mapping.

    Unsupported layouts fail closed; actionlint remains the YAML syntax owner.
    Comments and inputs on other steps cannot satisfy this retained policy.
    """
    ...
    scorecard_steps = [
        ...
        for step in steps
        if re.search(
            r"(?m)^        uses: ossf/scorecard-action@[^\s]+(?:\s+#.*)?$",
            step,
        )
    ]
